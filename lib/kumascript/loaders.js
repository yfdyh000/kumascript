...
var BaseLoader = ks_utils.Class({
    ...
    get: function (name, cb) {
        var $this = this;
        log.info("BaseLoader.get, name: " + name);
        try {
    ...
    cache_fetch: function (name, cb) {
        ...
        this.memcached.get(key, function (err, result) {
            log.info("cache_fetch, get callback, key: " + key);
            log.info("cache_fetch, get callback, err: " + err);
            log.info("cache_fetch, get callback, result: " + result);
            if (err || !result) {
                cb(err, null);
            } else {
    ...
    cache_store: function (name, source, cb) {
        ...
        this.memcached.set(key, cache_source, function (err, result) {
            log.info("cache_store, set callback, key: " + key);
            log.info("cache_store, set callback, err: " + err);
            log.info("cache_store, set callback, result: " + result);
