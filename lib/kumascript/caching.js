...
module.exports.ResponseCache = ks_utils.Class({
    ...
    set: function (key, max_age, headers, body, meta, next) {
        ...
        $this.memcached.set(key + ':headers', headers, function (e, r) {
            $this.memcached.set(key + ':body', body, function (e, r) {
                $this.memcached.set(key + ':meta', meta, function (e, r) {
...
function handle304 (memcached, key, timeout, resp, did_validate, cb) {
    memcached.get(key('body'), function (err, result) {
        ...
        if (!did_validate) {
            ...
            var now = (new Date()).getTime();
            memcached.set(key('validated_at'), now.toString(),
                function (err, result) {
                    cb(null, resp, source, true);
                }, timeout
            );
...
function handle200 (memcached, key, timeout, resp, source, cb) {
    ...
    memcached.set(key('body'), cache_source, function (err, result) {
        memcached.set(key('last_mod'), last_mod, function (err, result) {
            memcached.set(key('validated_at'), validated_at.toString(), function (err, result) {
